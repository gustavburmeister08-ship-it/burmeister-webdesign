...
const READ_LABEL = { de: "Lesen", en: "Read" } as const;
...
export function RatgeberCard({ article }: { article: RatgeberArtikel }) {
  ...
  const href =
    locale === "en"
      ? `/en/ratgeber/${article.slug}`
      : `/ratgeber/${article.slug}`;
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-1 flex-col p-6">
        ...
        <h3 className="mt-4 font-serif text-xl text-foreground">{article.title[locale]}</h3>
        <p className="mt-2 text-sm leading-relaxed text-foreground/65">{article.excerpt[locale]}</p>
        <div className="mt-auto flex items-center justify-between pt-6">
          <span className="flex items-center gap-1.5 text-xs text-muted-foreground">
            <Clock size={13} /> {article.readTime[locale]}
          </span>
          <span className="flex items-center gap-1.5 text-sm font-medium text-primary">
            {READ_LABEL[locale]} <ArrowRight size={14} />
          </span>
        </div>
      </div>
    </a>
